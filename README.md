...
**提示词刻意留白。** 发给模型的提示里不给画面示范、不写构图要求，也不提醒它别拿文字冒充图形——这些引导会替弱模型兜底，把最该被测出来的差异抹平。SVG 提示词因此只保留渲染所必需的代码约束（`xmlns`、标签闭合、坐标范围、不引用外部资源）。如果把"画出好看的图"当成目标去优化这些提示词，测的就不再是模型能力了。
